...
mflibs::verify::email() {
  ...
  if [[ $1 =~ $email_re ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is a valid email\n"
    return 0
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is an invalid email\n"
  return 1
}
...
(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$"
  if [[ "${1}" =~ $re ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is a valid ipv6 address\n"
    return 0
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is an invalid ipv6 address\n"
  return 1
}
...
mflibs::verify::ipv4() {
  ...
  if [[ ! "${verify_ip}" =~ ^[0-9]+(\.[0-9]+){3}$ ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is an invalid ipv4 address\n"
    return 1
  fi
  declare octet
  for octet in {0..3}; do
  ...
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is a valid ipv4 address\n"
  return 0
}
...
mflibs::verify::alpha() {
  ...
  declare re='^[[:alpha:]]+$'
  if [[ "${1}" =~ $re ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is alpha\n"
    return 0
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is not alpha\n"
  return 1
}
...
mflibs::verify::numeric() {
  ...
  if [[ "${1}" =~ ^[0-9]+$ ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is numeric\n"
    return 0
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is not numeric\n"
  return 1
}
...
mflibs::verify::alpha_dash() {
  ...
  declare re='^[[:alpha:]_-]+$'
  if [[ "${1}" =~ $re ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: $1 is alpha_dash\n"
    return 0
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 3)1$(tput sgr0)]: $1 is not alpha_dash\n"
  return 1
}
...
mflibs::verify::sudo() {
  if [[ $(id -u) -ne 0 ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 1)1$(tput sgr0)]: user is not root or sudo\n"
    return 1
  fi
  [[ " ${MFLIBS_LOADED[*]} " =~ verbose || " ${MFLIBS_LOADED[*]} " =~ debug ]] && echo -ne "[$(tput setaf 2)0$(tput sgr0)]: user is root or sudo\n"
}
