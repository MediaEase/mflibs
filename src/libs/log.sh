...
mflibs::log::init() {
  ...
  if [[ ! -d "$(dirname "${mflibs_log_location}")" ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose ]] && echo -ne "\033[38;5;44m[INFO]\e[0m - creating log location: $(dirname "${mflibs_log_location}")\n"
    mkdir -p "$(dirname "${mflibs_log_location}")"
  fi
  if [[ -f "${mflibs_log_location}" && $(wc -c "${mflibs_log_location}" | awk '{print $1}') -gt 10000 ]]; then
    [[ " ${MFLIBS_LOADED[*]} " =~ verbose ]] && echo -ne "\033[38;5;44m[INFO]\e[0m - rotating ${mflibs_log_location} to ${mflibs_log_location}.1\n"
    mv -f "${mflibs_log_location}" "${mflibs_log_location}.1"
  fi
  touch "${mflibs_log_location}" || { echo -ne "\033[38;5;203m[ERROR]\e[0m[1] - unable to touch ${mflibs_log_location}\n" && exit 1; }
